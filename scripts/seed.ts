// scripts/seed.ts → RIBELLA VERMOUTH – COMPLETO CON MATERIAS PRIMAS, VENTAS REALES Y TODO
...
async function main() {
  ...
  // 8. MATERIAS PRIMAS (botánicos, vino, alcohol, etc)
  ...
  // 10. VENTAS REALES DEL 2 AL 5 DE DICIEMBRE 2025
  const ventas = [
    {
      fecha: '2025-12-02',
      tipo: 'combo',
    ...
    { fecha: '2025-12-02', tipo: 'combo', cant: 4, monto: 0 }, // degustación
    { fecha: '2025-12-02', tipo: 'bianco', cant: 1, monto: 0 }, // rotura
    {
      fecha: '2025-12-03',
      tipo: 'combo',
      cant: 2,
    ...
    { fecha: '2025-12-04', tipo: 'combo', cant: 2, monto: 0 }, // donación
    {
      fecha: '2025-12-04',
      tipo: 'rosso2',
      cant: 2,
  ...
  for (const v of ventas) {
    ...
    if (v.monto > 0) {
      ...
      // Degustación, rotura, donación → solo restar stock
      if (v.tipo === 'combo') {
        await prisma.stock.updateMany({
          where: { productId: rosso.id },
  ...
  // 11. COMPOSICIÓN DE PRODUCTOS - MATERIAS PRIMAS POR BOTELLA (750ml)
  console.log('Cargando recetas reales de RibellA Rosso y Bianco...');

  // Buscamos las materias primas por SKU
  ...
  // === RIBELLA VERMOUTH ROSSO - Receta por botella de 750ml ===
  await prisma.productMaterial.createMany({
    data: [
      // Base líquida (en litros)
      {
        productId: rosso.id,
        rawMaterialId: vinoMalbec!.id,
        quantity: 0.5625,
        unitOfMeasure: 'litro',
      }, // 562.5 ml vino (75% de 750ml)
      {
        productId: rosso.id,
      ...
      }, // 135 ml alcohol
      {
        ...
        quantity: 0.0525,
        unitOfMeasure: 'litro',
      }, // 52.5 ml mosto concentrado

      // Botánicos (en gramos por botella)
      {
        productId: rosso.id,
  ...
  // === RIBELLA VERMOUTH BIANCO - Receta por botella de 750ml ===
  await prisma.productMaterial.createMany({
    data: [
      // Base más suave
      {
      ...
      // Botánicos más florales y frescos
      {
        productId: bianco.id,
        rawMaterialId: ajenjo!.id,
        quantity: 0.6,
  ...
  // ===================================================================
  // 12. PROVEEDORES DE EMPAQUE + INSUMOS + GASTOS REALES
  // ===================================================================

  console.log(
    'Cargando proveedores de empaque, gastos operativos y costos reales...',
  );
  ...
  // --- ASIGNAR EMPAQUE A CADA PRODUCTO (por botella terminada) ---
  ...
  await prisma.productPackaging.createMany({
    data: [
      ...
      },
      {
        ...
        quantityPerUnit: 1 / 6,
      }, // 1 caja cada 6 botellas

      // Lo mismo para Bianco
      {
        productId: bianco.id,
        packagingMaterialId: botella!.id,
        quantityPerUnit: 1,
  ...
  // --- GASTOS REALES MENSUALES (diciembre 2025) ---
  ...
  console.log(
    'Empaque, proveedores, gastos fijos y variables → todo listo para rentabilidad real',
  );
  ...
  console.log(
    'Listo para calcular costos, producir lotes y dominar el mercado',
  );
  console.log('RIBELLA VERMOUTH CARGADA 100%');
  console.log(
    'Materias primas, productos, stock, clientes y todas las ventas reales del 2 al 5 dic 2025',
  );
